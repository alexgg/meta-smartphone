...
PACKAGE_ARCH_titan = "${MACHINE_ARCH}"

SRC_URI_append_athene = " file://70-athene.rules"
SRC_URI_append_titan = " file://70-titan.rules"

do_install_append_athene() {
    install -m 0644 ${WORKDIR}/70-athene.rules ${D}${sysconfdir}/udev/rules.d/70-athene.rules
}

do_install_append_titan() {
    install -m 0644 ${WORKDIR}/70-titan.rules ${D}${sysconfdir}/udev/rules.d/70-titan.rules
}
